...
% the dataset is in the colon.mat 
...
% If you want to run SparseEA in a new data set:
...
for data = 1:length(dataNameArray)
    ...
    for i = 1:iterator
        ...
        % Define lower and upper bounds, and variable types for SparseEA
        lower = zeros(1, size(train_F, 2));  % Assuming all variables have a lower bound of 0
        upper = ones(1, size(train_F, 2));   % Assuming all variables have an upper bound of 1
        variableTypes = ones(1, size(train_F, 2));  % Assuming all variables are binary

        % Call SparseEA with the correct parameters
        [outcome{i,1},outcome{i,2},outcome{i,3},outcome{i,4},outcome{i,5},outcome{i,6},outcome{i,7}] = SparseEA(train_F, train_L, lower, upper, variableTypes, sizep, i);
